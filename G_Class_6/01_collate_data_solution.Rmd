...
# let's sanity-check the data to make sure the proportion of NAs for a given marker
# is not too high, also we can find the longest sequence of NAs for a given marker
nas_by_marker <- traj_data %>% 
  select(contains("_x")) %>% 
  is.na() %>% 
  colSums() %>% 
  as_tibble() %>%
  mutate(marker = marker_names) %>%
  arrange(desc(value))
...
# first we can define a function to gap fill a single column
gap_fill <- function(x) {
...
# now we can apply this function to each column
traj_data <- traj_data %>% 
  mutate_at(vars(contains("_x")), gap_fill) %>% 
  mutate_at(vars(contains("_y")), gap_fill) %>% 
  mutate_at(vars(contains("_z")), gap_fill)
